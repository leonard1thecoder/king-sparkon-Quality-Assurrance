...
public final class SvcApiScenarioReader {

    private SvcApiScenarioReader() {
    }

    public static List<ApiScenario> read(String resourcePath) {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(stream, StandardCharsets.UTF_8))) {
            return reader.lines()
                    ...
                    .map(SvcApiScenarioReader::toApiScenario)
                    .toList();
        } catch (IOException exception) {
            throw new IllegalStateException("Unable to read API scenario file: " + resourcePath, exception);
        }
    }
