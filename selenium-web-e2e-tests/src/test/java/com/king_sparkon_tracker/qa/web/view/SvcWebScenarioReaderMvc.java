...
public final class SvcWebScenarioReaderMvc {

    private SvcWebScenarioReaderMvc() {
    }

    public static List<WebScenarioModel> read(String resourcePath) {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(stream, StandardCharsets.UTF_8))) {
            return reader.lines()
                    ...
                    .map(SvcWebScenarioReaderMvc::toScenario)
                    .toList();
        } catch (IOException exception) {
            throw new IllegalStateException("Unable to read web scenario file: " + resourcePath, exception);
        }
    }

    private static WebScenarioModel toScenario(String line) {
        String[] parts = line.split("\\|", -1);
        if (parts.length != 5) {
            throw new IllegalArgumentException("Invalid web scenario line. Expected 5 columns but got " + parts.length + ": " + line);
        }

        return new WebScenarioModel(
                required(parts[0], "id"),
